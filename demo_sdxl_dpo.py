...
pipe = StableDiffusionXLPipeline.from_pretrained(dpo_model_id, torch_dtype=torch.float16, variant="fp16", add_watermarker=False, use_safetensors=True).to(sdxl_device)
unet = UNet2DConditionModel.from_pretrained(dpo_unet_id, subfolder="unet", torch_dtype=weight_dtype)
pipe.unet = unet
...
def generate_images(text_prompt, seed):
    ...
    dpo_image = dpo_pipe(text_prompt, generator=torch.Generator().manual_seed(seed), num_inference_steps=50, guidance_scale=7.5).images[0]
    ip_feat = get_ip_feat(dpo_image, ip_image_processor, ip_clip, ip_resampler)
    ...
    return dpo_image, generated_image[0]

with gr.Blocks() as demo:
    with gr.Row():
        prompt = gr.Textbox(label="Prompt", value="The rectangular mirror was hung above the white sink.")
        seed = gr.Number(label="Seed", value=115)
    with gr.Row():
    ...
    gr.Examples(
        examples=[
            ["The rectangular mirror was hung above the white sink.", 115],
            ["The city of London on Mars", 305],
            ],
        inputs=[prompt, seed],
